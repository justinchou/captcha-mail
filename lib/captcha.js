...
class Captcha extends EventEmitter {
  ...
  init () {
    ...
    if (this.config.expireSeconds && Is.number(this.config.expireSeconds) && this.config.expireSeconds > MIN_EXPIRE_SECONDS) this.expireSeconds = parseInt(this.config.expireSeconds, 10);
  ...
  async generateCaptcha (email, len) {
    ...
    if (this.expireSeconds > MIN_EXPIRE_SECONDS) {
      await this.redis.setAsync(`${this.cachePrefix}${email}`, `${captcha}`, 'EX', this.expireSeconds);
    } else {
